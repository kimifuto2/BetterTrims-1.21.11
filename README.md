...
| 下界合金 Netherite | 斩首/火免/甲韧 | 每件 +2 护甲、+1 韧度；**免疫火焰伤害**，物品不会被烧毁；攻击有概率**追加穿透伤害**并**掉落生物头颅**（凋零骷髅头/骷髅头/僵尸头/苦力怕头/猪灵头） |
...
| 材质物品 | 效果 | 状态 |
|---|---|---|
| 荧石粉 | 发光；**下界中**每件 +攻击/攻速/移速/减伤 | ✅ 已实现 |
| 下界砖 | 防火；每件 **-5% 火焰伤害** | ✅ 已实现 |
| 海晶碎片 | 水下呼吸；对**水生生物**每件 +2 伤害 | ✅ 已实现 |
| 附魔金苹果 | +6/12/16/20 生命上限；每件 **-5% 伤害**（4 件 -20%）；**穿 2 件获得生命恢复 I，穿全套 4 件生命恢复 II** | ✅ 已实现 |
