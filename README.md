...
| 材质物品 | 效果 | 状态 |
|---|---|---|
| 荧石粉 | 发光；**下界中**获得攻击/移速/护甲加成 | ✅ 已实现 |
| 下界砖 | 抗火 + 火焰伤害减免 | ✅ 已实现 |
| 海晶碎片 | 水下呼吸 | ✅ 已实现 |
| 附魔金苹果 | +生命上限；**每秒无视饥饿恢复生命** | ✅ 已实现 |
| 火焰弹 | 攻击点燃敌人（4 秒/件） | ✅ 已实现 |
| 粘液球 | -击退抗性（更易被击退）+ 攻击击退 | ✅ 已实现 |
| 紫颂果 | +闪避概率 | ⏳ 待实现（需自定义逻辑） |
| 煤炭 | 受击时烧炼物品 | ⏳ 待实现（需自定义逻辑） |
| 龙息 | 状态效果共享范围 | ⏳ 待实现（需自定义逻辑） |
| 回响碎片 | 死亡回溯 + 冷却 | ⏳ 待实现（需自定义逻辑） |
| 末影珍珠 | 弹射物闪避 + 水中伤害 | ⏳ 待实现（需自定义逻辑） |
